...
Ltac pre_simpl_hyp_perm H :=
  ...
  try (rewrite app_assoc in H; apply Permutation_cons_app_inv in H);
  try (rewrite app_comm_cons in H; apply Permutation_cons_app_inv in H);
  try (rewrite app_comm_cons in H; rewrite app_assoc in H; apply Permutation_cons_app_inv in H).
Ltac simpl_hyp_perm H :=
  list_simpl in H; try pre_simpl_hyp_perm H;
  try (apply Permutation_sym in H; pre_simpl_hyp_perm H; apply Permutation_sym in H).
...
Ltac perm_rot :=
  ...
  eapply Permutation_trans; [ apply Permutation_app_rot | ].
...
Ltac Permutation_solve :=
  match goal with
  | |- Permutation _ _ =>
    list_simpl; try simpl_hyp_perm_all;
    cons2app in *; try simpl_hyp_perm_all;
    first [
    ...
    | match goal with
      | H:Permutation _ _ |- Permutation _ _ =>
            ...
            try (apply Permutation_sym; apply (Permutation_trans H); perm_run; fail);
            try (apply Permutation_sym in H; apply (Permutation_trans H); perm_run; fail);
...
with perm_run :=
  ...
  | H:Permutation ?l1 _ |- Permutation (?l1 ++ _) _
       => eapply Permutation_trans; [ apply Permutation_app_tail; apply H | ]
  | H:Permutation _ ?l1 |- Permutation (?l1 ++ _) _
       => apply Permutation_sym in H;
          eapply Permutation_trans; [ apply Permutation_app_tail; apply H | ]
  | |- Permutation (_ ++ _ ++ _) _ => perm_rot
  | |- Permutation (_ ++ _ ) _ => eapply Permutation_trans; [ apply Permutation_app_comm | ]
  | H:Permutation ?l1 _ |- Permutation ?l1 _
       => eapply Permutation_trans; [ apply H | ]
