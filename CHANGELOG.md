...
* add new files
  * `SubListT`: a version of `sublist` with output in `Type`
* add new lemmas
  * `funtheory`: `map_bijective`
  * `List_more`: `Exists_elt`, `Forall2[T]_preorder`, `ForallT_app_iffT`, `fold_right_unit`
  * `dectype`: cosntruction of a `DecType` from the data generated by `Scheme Equality`
  * `Shuffle[T]`: `sublist[T]_shuffle[T]`, `sublist[T]_cons_inv`
  * `SubList[T]`: `sublist[T]_sgt`, `sublist[T]_in[T]_extend`, `sublist[T]_filter`
* list tactics
...
* notations
  * create module `LogicNotations` with notation for `iffT`
  * add a notation for `l ++ a :: nil` to `ListNotations`
  * add a notation for function composition
...
* correct bad namings
  ...
  * rename `Forall_map` into `Forall_exists_map` (name clash with Stdlib)
