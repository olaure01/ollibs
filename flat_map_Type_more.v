  ...
  + destruct a as (n, l'); simpl in Heq ; rewrite app_comm_cons in Heq.
    ...
    * apply IHL in Heq1 ; destruct Heq1 as [[Heqa | Heqb] | [Heq Heqc]].
      ...
      -- left ; right.
         ...
         exists ((n, l') :: L', L'', n', l'', l''') ; simpl ; subst ; split ; [ | split ]...
         simpl.
         rewrite ? app_assoc...
      -- right.
         ...
         exists ((n, l') :: L', L'', n' , l'') ; simpl ; subst ; split ; [ | split ; [| split] ]...
Qed.
...
- destruct a as (n, a); simpl in HP.
  ...
  + exists (L0 ++ (n0 , l0) :: (n, l1) :: L1, l') ; split ; [ | split ] ; simpl...
...
{' (l1',l2',l3',l4',l',L') : _ &  prod (l1 ++ map f lp1 ++ l2 = l' ++ flat_map (fun '(p1,p2) => g p1 :: p2) L')
...
- app_vs_app_flat_map_inv Heq1.
  + exists (lp1,lp2,l1,l ++ flat_map (fun '(p1,p2) => (app (map f l0) p2)) L,l1 ++ map f lp1 ++ l,L) ;
  ...
  + destruct (perm_Type_f_app_flat_map f l0 _ _ _ _ HP Heq2) as [[L' l'] (Hnil' & HeqL' & HPL')] ;
    ...
    induction L' using rev_rect;
      ...
      | clear IHL' ].
    destruct x as (n' , x).
    exists (Lp',Lp,l1,l4 ++ flat_map (fun '(p1,p2) => (app (map f l0) p2)) L1,l1 ++ l',L' ++ (n' , x ++ l4) :: L1) ;
      list_simpl ; split ; [ | split ; [ | split ]] ;
...
- app_vs_app_flat_map_inv Heq2.
  + exists (lp1,lp2,
            l ++ flat_map (fun '(p1,p2) => app (map f l0) p2) (L0 ++ (n , l3) :: nil),l1 ++ flat_map (fun '(p1,p2) => app (map f l0) p2) L1,
  ...
  + destruct (perm_Type_f_app_flat_map f l0 _ _ _ _ HP Heq1) as [[L' l''] (Hnil' & HeqL' & HPL')] ;
    ...
    destruct x as (n' , x).
    exists (Lp',Lp,
            l ++ flat_map (fun '(p1,p2) => app (map f l0) p2) L0 ++ map f l0 ++ l3,l5 ++ flat_map (fun '(p1,p2) => app (map f l0) p2) L2,
  ...
  + destruct (perm_Type_f_app_flat_map f l0 _ _ _ _ HP Heq1) as [[L' l''] (Hnil' & HeqL' & HPL')] ;
    ...
    assert (Permutation_Type Lp' Lp) as HPp
      by (rewrite HeqLp in HPL' ; rewrite HeqLp' in HPL' ; apply (Permutation_Type_map_inv_inj _ Hinj _ _ HPL')).
    exists (Lp',Lp,
            l ++ flat_map (fun '(p1,p2) => app (map f l0) p2) L0 ++ map f l0 ++ l3,flat_map (fun '(p1,p2) => app (map f l0) p2) L2,
            l,L0 ++ (n , l3 ++ l'') :: L' ++ L2) ;
      list_simpl ; split ; [ | split ; [ | split ]] ;
      try rewrite HeqL'; try rewrite <- HeqLp' ; try rewrite <- HeqLp ;
      rewrite ? flat_map_app ; list_simpl ; rewrite ? flat_map_app ; list_simpl...
- app_vs_flat_map_inv Heq2.
  + rewrite <- (app_nil_l (flat_map _ _)) in Heq1.
    ...
    * destruct x as (n', x).
      ...
      exists (Lp',Lp,
              l ++ flat_map (fun '(p1,p2) => app (map f l0) p2) L0 ++ map f l0 ++ x0,l3 ++ flat_map (fun '(p1,p2) => app (map f l0) p2) L2,
  ...
  + rewrite <- (app_nil_l (flat_map _ _)) in Heq1.
    ...
    * destruct x as (n' , x).
      exists (Lp',Lp,
              l ++ flat_map (fun '(p1,p2) => app (map f l0) p2) L0 ++ map f l0 ++ x, flat_map (fun '(p1,p2) => app (map f l0) p2) L2,
              l,L0 ++ (n', x ++ l'') :: L' ++ L2) ;
        list_simpl ; split ; [ | split ; [ | split ]] ;
        try rewrite HeqL'; try rewrite <- HeqLp' ; try rewrite <- HeqLp ;
        rewrite ? flat_map_app ; list_simpl ; rewrite ? flat_map_app ; list_simpl...
...
Proof with try assumption ; try reflexivity.
  ...
  - destruct s as (((((L1 & L2) & n) & l3) & l4) & H).
    left.
    exists (map snd L1, map snd L2, l3, l4).
    ...
    + clear - H2.
      ...
      * simpl.
        ...
        replace (map snd (map (fun x => (1, x)) L)) with L...
        clear; induction L...
        simpl; rewrite <- IHL...
      * destruct L; inversion H2.
        simpl.
    ...
    + replace (flat_map (cons A) (map snd L2)) with (flat_map (fun '(p1,p2) => A :: p2) L2)...
      clear; induction L2...
      destruct a; simpl; rewrite IHL2...
  - right.
...
Proof with try assumption ; try reflexivity.
  ...
  - replace (flat_map (cons A) (map snd L')) with (flat_map (fun '(p1,p2) => A :: p2) L')...
    clear; induction L'...
  ...
  - replace (flat_map (app lw0) (map snd L')) with (flat_map (fun '(p1,p2) => lw0 ++ p2) L')
    ...
    replace (flat_map (app lw0) L) with (flat_map (fun '(p1,p2) => lw0 ++ p2)
                                                  (map (fun p => (1, p)) L)) by (clear; induction L; simpl; try rewrite IHL; reflexivity)...
Qed.  
...
Proof with try assumption ; try reflexivity.
  ...
  split with (l1', l2', l3', l4', l', map snd L').
  split; [ | split; [ | split]]...
  - rewrite H1.
