...
Ltac pre_simpl_hyp_perm_Type H :=
  ...
  try (rewrite app_assoc in H; apply Permutation_Type_cons_app_inv in H);
  try (rewrite app_comm_cons in H; apply Permutation_Type_cons_app_inv in H);
  try (rewrite app_comm_cons in H; rewrite app_assoc in H; apply Permutation_Type_cons_app_inv in H).
Ltac simpl_hyp_perm_Type H :=
  list_simpl in H;
  try pre_simpl_hyp_perm_Type H;
  try (apply Permutation_Type_sym in H; pre_simpl_hyp_perm_Type H; apply Permutation_Type_sym in H).
...
Ltac perm_Type_rot :=
  ...
  eapply Permutation_Type_trans; [ apply Permutation_Type_app_rot | ].
...
Ltac Permutation_Type_solve :=
  ...
  | |- Permutation_Type _ _ =>
    list_simpl; try simpl_hyp_perm_all_Type;
    cons2app in *; try simpl_hyp_perm_all_Type;
    first [
    ...
    | match goal with
      | H:Permutation_Type _ _ |- Permutation_Type _ _ =>
            ...
            try (apply Permutation_Type_sym; apply (Permutation_Type_trans H); perm_Type_run; fail);
            try (apply Permutation_Type_sym in H; apply (Permutation_Type_trans H); perm_Type_run; fail);
...
with perm_Type_run :=
  ...
  | H:Permutation_Type ?l1 _ |- Permutation_Type (?l1 ++ _) _
       => eapply Permutation_Type_trans; [ apply Permutation_Type_app_tail; apply H | ]
  | H:Permutation_Type _ ?l1 |- Permutation_Type (?l1 ++ _) _
       => apply Permutation_Type_sym in H;
          eapply Permutation_Type_trans; [ apply Permutation_Type_app_tail; apply H | ]
  | |- Permutation_Type (_ ++ _ ++ _) _ => perm_Type_rot
  | |- Permutation_Type (_ ++ _ ) _ => eapply Permutation_Type_trans; [ apply Permutation_Type_app_comm | ]
  | H:Permutation_Type ?l1 _ |- Permutation_Type ?l1 _
       => eapply Permutation_Type_trans; [ apply H | ]
